...
export default function CourseSlider() {
  ...
  return (
    <div className=" max-w-7xl mx-auto flex flex-col gap-2 py-8 px-6">
      {/* slide bar */}
      <div className="max-w-7xl flex flex-row gap-2 ">
        <div className="w-2/3">
          ...
          <div className="text-white  bg-[#111827] max-w-7xl  grid grid-cols-2 gap-4">
            {data?.data?.sections[4].values.map((item) => (
              <div className="flex flex-row" key={item.id}>
                <div className="px-4 py-6 h-20 w-24">
                  <Image
                    src={item.icon}
                    alt={item.title}
                    width={30}
                ...
                <div className="flex flex-col py-4 ">
                  ...
                  <h3 className="pb-1 text-sm text-gray-300">
                    {item.subtitle}
                  </h3>
                </div>
              </div>
          ...
          <div
            className="text-white my-6 p-10  flex flex-row h-[300px] bg-cover bg-center rounded-xl"
            style={{
        ...
        <div className="">
          <IeltsCourseBanner></IeltsCourseBanner>
        </div>
      </div>
      ...
      <div className="max-w-7xl md:w-2/3 ">
        ...
        <div className="p-8 border-1  rounded-xl border-gray-300 grid md:grid-cols-2">
          {data?.data?.sections[6].values.map((item) => (
            <div className=" flex flex-row gap-4 p-2" key={item.id}>
              <GrCheckmark className="text-blue-500 font-medium  h-12 w-12"></GrCheckmark>
            ...
            >
              <div className="w-[70%] p-2">
                ...
                {item.checklist.map((list, index) => (
                  <div className="py-2  flex flex-row gap-4" key={index}>
                    <GrCheckmark className="text-blue-500 font-medium "></GrCheckmark>
                    <p className="text-md text-gray-600 font-medium">{list}</p>
                  </div>
                ))}
              </div>
              <div className="w-[30%]">
          ...
          <div className="flex flex-row justify-between">
            ...
            <div className="flex flex-row gap-4">
              <BiSolidSkipPreviousCircle
                ...
                className={`text-[40px] transition-transform "text-black"
                  }`}
              />

              <BiSolidSkipPreviousCircle
                onClick={videoScrollRight}
                className={`text-[40px] rotate-180 transition-transform ${"text-black"}`}
              />
            </div>
          </div>
                    ...
                    <div className="relative aspect-video rounded-lg overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                        <iframe
                          ...
                          allowFullScreen
                        >
                          <button className="w-14 h-14 rounded-full bg-white/90 hover:bg-white transition hover:scale-110">
                            ▶
                          </button>
                        </iframe>
                      </div>
